...
func main() {
	...
	fmt.Println("🕵️ Core Rule Processing Engine online. Awaiting real-time transaction swipes...")
	...
	for {
		...
		if err := json.Unmarshal(msg.Value, &tx); err != nil {
			fmt.Printf("⚠️ Skipped malformed transaction message schema: %v\n", err)
			continue
		}
...
func evaluateTransactionTelemetry(ctx context.Context, rdb *redis.Client, tx Transaction) {
	...
	// =========================================================================
	// ZONE 2: RULE 1 - Geospatial Baseline Context Check (Redis Hashes)
	// =========================================================================
	...
	if err == nil && len(profile) > 0 {
		...
		if homeCity != "" && tx.Location != homeCity {
			...
			fmt.Printf("⚠️ [GEOSPATIAL WARN] Card %s -> Swiped at %s location.\n", tx.CardNumber, tx.Location)
		}
	}
...
func synthesizeAndPublishAlert(tx Transaction, violations []string) {
	...
	if err != nil {
		fmt.Printf("❌ Critical log append failure on fraud alert channel: %v\n", err)
	} else {
